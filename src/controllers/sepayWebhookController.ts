...
const extractSessionId = (content: string) => {
  ...
  // Original format generated by app: BF|s:<session>|p:<slug>
  const standardMatch = normalized.match(/s:([a-z0-9]+)/i);
  if (standardMatch?.[1]) return standardMatch[1];

  // Banks can strip separators and hyphens, e.g. BFsmmfadu7hphrenterprise
  const compactMatch = normalized.match(/BFs([a-z0-9]+)p(?:free|hrpro|hrenterprise)/i);
  if (compactMatch?.[1]) return compactMatch[1];

  // Fallback: BF<sId> if package suffix was lost
  const looseMatch = normalized.match(/BFs([a-z0-9]{6,})/i);
